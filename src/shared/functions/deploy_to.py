...
class DeployToFunction(BaseFunction):
    ...
    async def execute(
        ...
        **kwargs: Any
        ...
        try:
            ...
            selected_clusters = self._filter_clusters(all_clusters, target_clusters, cluster_labels)
            ...
            results = await self._deploy_to_clusters(
                selected_clusters, filename, resource_type, resource_name,
                image, cluster_images, namespace, kubeconfig
            )
    ...
    async def _list_available_clusters(self, kubeconfig: str, remote_context: str) -> Dict[str, Any]:
    ...
    async def _deploy_to_clusters(
        ...
        for cluster in clusters:
            result = await self._deploy_to_cluster(
                cluster, filename, resource_type, resource_name,
                image, cluster_image_map, namespace, kubeconfig
            )
